...
REM Ensure JetBrains-style CJK UI font is present. The .ttf file is not
REM committed to the repo; it is fetched on first build via a China mirror.
REM Only the Regular variant is embedded at runtime; Fyne synthesizes bold
REM from it.
...
if not exist "%FONT_REGULAR%" (
    ...
    powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; [Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; $url='%FONT_URL%'; $zip='%FONT_ZIP%'; Write-Host ('Downloading ' + $url); Invoke-WebRequest -UseBasicParsing -Uri $url -OutFile $zip; if (Test-Path '%FONT_DIR%\tmp-fonts') { Remove-Item -Recurse -Force '%FONT_DIR%\tmp-fonts' }; $null=New-Item -ItemType Directory -Path '%FONT_DIR%\tmp-fonts'; Expand-Archive -Path $zip -DestinationPath '%FONT_DIR%\tmp-fonts' -Force; Copy-Item '%FONT_DIR%\tmp-fonts\MapleMono-CN-Regular.ttf' '%FONT_REGULAR%' -Force; Remove-Item -Recurse -Force '%FONT_DIR%\tmp-fonts'"
    if errorlevel 1 (
        echo ERROR: failed to download Maple Mono CN font from %FONT_URL%
