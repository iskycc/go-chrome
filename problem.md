...
| 31 | **字体太细了**（user-reported） | 嵌入的 `CascadiaNextSC.wght.ttf` 是 variable font，fvar wght axis 默认 = 200 (ExtraLight)，OS/2 usWeightClass = 200。Fyne 2.7.4 的 `loadMeasureFont` 不调用 `SetVariations`，按 fvar 默认坐标插值出字形 → 全部字符（包括 CJK）渲染成 ExtraLight 粗细；450 仍偏细，按用户反馈提到 600 (SemiBold) | 写一个 `scripts/bake-font` 工具读 TTF 二进制，把 fvar 的 wght axis Default 和 OS/2 usWeightClass 改成目标粗细。生成 `assets/fonts/CascadiaNextSC-600.ttf`，`assets/embed.go` 改用新文件。`go-text/typesetting` 的 glyph outline 插值走新默认 600 → 字符笔画明显加重 |
...
> 注：以下 P0/P1 问题已在 `main` 分支完成修复，对应关系见“已修复的问题”表格 #20~#27。
> 这里保留原始问题描述用于追溯，状态已全部勾选完成。

- [x] P0-1. 顶部状态栏缺少字段名，用户看不懂状态含义
...
- [x] P0-2. 程序启动后不恢复上次选择的流程，也不默认打开流程
...
- [x] P0-3. 没有“下载模板 / 模板库”功能，只能导入本地 JSON
...
## UI 三次优化完成项（2026-06-12 UI 三次优化）

- [x] 中英文字体统一为 JetBrains Mono 风格 CJK 字体（Maple Mono CN Regular / Medium）
- [x] 全局 UI 不再使用过细字重，标题与主按钮使用 Medium 字重
- [x] 字体 .ttf 文件不纳入版本库，由构建脚本通过国内镜像自动下载
- [x] “关闭托管 Chrome”与“启动 Chrome”在全局工具栏同组可见
- [x] “停止当前流程”常驻全局工具栏，空闲禁用、运行中启用
- [x] 运行详情页删除重复关闭 Chrome 入口
- [x] 环境配置导入/导出支持默认文件名、JSON 过滤、敏感变量提示
- [x] 环境导入重名时自动追加后缀，不覆盖现有环境

## UI 四次优化完成项（2026-06-12 UI 四次优化）

- [x] 建立统一弹框尺寸规范 helper（internal/ui/dialogs.go）
- [x] 环境配置 4 个表单弹框全部替换为统一 sized form，说明/变量值使用多行输入
- [x] 环境导入/导出文件弹框固定 720x520 尺寸
- [x] 流程导出弹框改为 NewFileSave + 默认文件名 + 720x520
- [x] 删除环境/变量/流程确认弹框支持长名称截断和自动换行
- [x] 导出敏感变量确认弹框文案更明确，按钮为"继续导出"/"取消"
- [x] 未应用步骤修改弹框改为三选一：应用并切换 / 放弃修改 / 取消
- [x] 未保存流程修改弹框流程名截断、尺寸调整为 560x220、按钮均分
- [x] 模板选择弹框列表项改为 truncating label，防止长名称溢出
- [x] 添加步骤弹框尺寸调整为 480x200

## UI 五次优化完成项（2026-06-12 UI 五次优化）

- [x] 新增通用右键菜单组件 contextMenuLabel（internal/ui/context_menu.go）
- [x] 流程库列表右键菜单：打开、运行、复制、导出、复制名称/ID、删除
- [x] 步骤表格右键菜单：编辑、从此运行、复制、删除（带确认）、上移、下移、启用/禁用、复制名称/XPath/输入（敏感确认）
- [x] 环境列表右键菜单：设为当前、重命名、复制、导出全部、复制名称/ID、删除
- [x] 环境变量表右键菜单：编辑、复制 key/value（敏感确认）、复制 ${env:KEY}、切换敏感标记、删除
- [x] 运行日志右键菜单：复制当前行、复制全部、清空、打开/复制产物目录
- [x] 历史记录列表右键菜单：查看详情、复制摘要/ID、打开/复制产物目录、筛选环境、按此环境重新运行
- [x] 右键操作前先选中对应行/单元格，避免误操作
- [x] 危险操作保留确认弹框，敏感值复制前二次确认

## UI 六次优化完成项（2026-06-12 UI 六次优化）

- [x] 新增专用 progressLabel 组件，将进度数字与步骤名拆分，确保数字永远可见
- [x] 修复全局工具栏进度文字被挤压成 `...` 的问题
- [x] 修复运行详情页进度文字被右侧按钮挤压成 `...` 的问题
